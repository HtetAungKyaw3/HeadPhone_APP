...
public class Antenna1562Fragment extends BaseFragment implements OnChartValueSelectedListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView = inflater.inflate(R.layout.fragment_antenna_1562, container,false);
        initUImember();
    ...
    AirohaMmiListener mAirohaMmiListener = new AirohaMmiListener() {
            ...
            mActivity.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (info.getStatus() == 0) {
                        ...
                        if(info.getDebugInfoLevel() > 0){
                            msg = msg + ", SyncStartCnt:" + info.getSyncStartCnt()
                                    + ", RecoveryCnt:" + info.getRecoveryCnt()
                                    + ", DropRecoveryCnt:" + info.getDropRecoveryCnt()
                                    + ", DspEmptyCnt:" + info.getDspEmptyCnt()
                                    + ", DspOutOfSyncCnt:" + info.getDspOutOfSyncCnt()
                                    + ", DspSeqLossWaitCnt:" + info.getDspSeqLossWaitCnt()
                                    + ", PiconetClock:0x" + String.format("%08x", info.getPiconetClock())
                                    + ", LowHeapDropCnt:" + info.getLowHeapDropCnt()
                                    + ", FullBufferDropCnt:" + info.getFullBufferDropCnt()
                                    + ", RemoteChMap:" + info.getRemoteChMapStr()
                                    + ", LocalChMap:" + info.getLocalChMapStr();
                        }

                        if (info.getDebugInfoLevel() > 1) {
                            msg += ", AclDataCnt:" + info.getAclDataCnt()
                                    + ", IfpDataCnt:" + info.getIfpDataCnt();
                        }
        ...
        @Override
        public void notifyDeviceType(boolean isTws) {
...
//                }
        ...
        @Override
        public void notifyPairingModeState(byte state) {

        }

        @Override
        public void notifyLinkHistory(int count, List<byte[]> deviceStatusList) {

        }

        @Override
        public void notifyLinkDeviceStatus(byte oldStatus, byte newStatus, byte[] address, byte[] name) {
