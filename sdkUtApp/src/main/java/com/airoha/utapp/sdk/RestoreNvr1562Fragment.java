...
public class RestoreNvr1562Fragment extends BaseFragment implements HostStateListener {
    private String TAG = RestoreNvr1562Fragment.class.getSimpleName();
    private String LOG_TAG = "[RestoreNvr] ";

    private static final int REQUEST_CHOOSE_NVR = 55;

    private RestoreNvr1562Fragment mFragment;
    private View mView;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView = inflater.inflate(R.layout.fragment_restore_nvr_1562, container, false);
                ...
                case R.id.btnRestoreNVR:
                    if (mSelectedFilePath.length() > 0) {
                        ...
                        if (mTargetRole == AgentPartnerEnum.BOTH) {
                            mTotalNvCount = ((AB1562FotaControl)AirohaSDK.getInst().getAirohaFotaControl()).getAirohaFotaMgrEx1562().startUpdateDualNvr(mSelectedFilePath, mSelectedFilePath);
                        } else {
                            mTotalNvCount = ((AB1562FotaControl)AirohaSDK.getInst().getAirohaFotaControl()).getAirohaFotaMgrEx1562().startUpdateSingleNvr(mSelectedFilePath, mTargetRole.getId());
                        }
                        if (mTotalNvCount == 0) {
...
//                    }
        ...
        @Override
        public void notifyGetAntennaInfo(byte role, AntennaInfo info) {
